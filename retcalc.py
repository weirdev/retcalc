...
def save_retirement_settings(
    scenario: "RetirementSettings", filepath: Optional[str] = None
):
    if filepath is None:
...
def retirement_value(
    ...
    while new_rs.t > 0:
        ...
        for ri, asset_alloc in enumerate(
            reversed(new_rs.asset_distribution.asset_allocations)
        ):
...
def rebalance_assets(asset_allocations: List[AssetAllocation]) -> None:
    ...
    total_assets = sum([get_and_clear_value(aa.asset) for aa in asset_allocations])
    if total_assets == 0:
        return
    ...
    for i, asset_alloc in enumerate(asset_allocations):
        ...
        if i + 1 == len(asset_allocations) or (
            priority < asset_allocations[i + 1].priority
            and (pc_total_min_value > 0 or pc_total_fraction > 0)
        ):
    ...
    assert abs(total_assets - sum([a.asset.value for a in asset_allocations])) < 0.001


def rsettings_print(retirementSettings: RetirementSettings):
    print(f"expenditure: ${retirementSettings.expenditure:,.2f}")
...
def asset_allocs_print(asset_allocations: List[AssetAllocation]):
    for alloc in asset_allocations:
        ...
        print(
            "\tPreferred fraction of priority class: "
...
def choose_priority(asset_allocations: List[AssetAllocation]) -> float:
    ...
    for i, alloc in enumerate(asset_allocations):
        ...
        options.append((f"Prioritize equally with {alloc.asset.name}", alloc.priority))
...
def safe_ret_expenditure_prompt():
    current_state = None
    if choose(
        [("Load current state from disk", True), ("Enter current state now", False)]
    ):
    ...
    if current_state is None:
        ...
        while add_asset:
            ...
            return_stdev = takefloat(
                f"Enter estimated {name} return standard deviation over this period",
                0,
                1,
            )
        ...
        inflation = (
            takefloat("Enter estimated mean inflation over this period", -1, 1),
            takefloat(
                "Enter estimated inflation standard deviation " + "over this period",
                0,
                1,
            ),
        )
...
def savings_required_for_expenditure_prompt():
    ...
    if choose(
        [
            ("Load retirement scenario from disk", True),
    ...
    if retirement_scenario is None:
        ...
        eret = (
            takefloat("Enter estimated mean equity return over this period", -1, 1),
            takefloat(
                "Enter estimated equity return standard deviation "
                + "over this period",
                0,
                1,
            ),
...
if __name__ == "__main__":
    ...
    prompt_fns = [
        ("Calculate max expenditure in retirement", safe_ret_expenditure_prompt),
        (
